...
export default function ArchiveView() {
  const { parts, products, unarchiveProduct, restartProduct, deleteProductPermanently } = useApp();
          ...
          return (
            <div key={item} className="product-card" style={{ opacity: 0.8 }}>
              <div className="product-header" style={{ cursor: 'default' }}>
                ...
                <button className="btn" style={{ fontSize: 12, padding: '4px 12px' }} onClick={() => unarchiveProduct(item)}>↑ restore</button>
                <button className="btn btn-primary" style={{ fontSize: 12, padding: '4px 12px', marginLeft: 4 }} onClick={() => { if (confirm(`Restart "${item}" from scratch? All part statuses and print counts will be reset to queue. Your inventory history is kept.`)) restartProduct(item); }}>⟳ restart</button>
